-- Complete BMW Parts Database Setup - FIXED VERSION
...
-- Create BMW Transmissions table
...
CREATE TABLE bmw_model_variants (
    ...
    transmission_id UUID REFERENCES bmw_transmissions(id) ON DELETE SET NULL,
...
('ZF8HP', 'ZF 8HP Automatic', 'Automatic', 8, 'ZF', 2009, NULL),
('ZF6HP', 'ZF 6HP Automatic', 'Automatic', 6, 'ZF', 2000, 2017),
('Getrag420G', 'Getrag 420G Manual', 'Manual', 6, 'Getrag', 2001, 2013),
('ZF8DCT', 'ZF 8-Speed DCT', 'DCT', 8, 'ZF', 2018, NULL),
('Aisin8AT', 'Aisin 8-Speed Automatic', 'Automatic', 8, 'Aisin', 2016, NULL);
...
SELECT 
    ...
    LEAST(COALESCE(c.production_end, 2024), 2020),
    CASE 
...
CREATE INDEX idx_bmw_compatibility_engines ON bmw_part_compatibility USING GIN(engine_codes);

-- Create materialized view for fast searching - FIXED VERSION
CREATE MATERIALIZED VIEW bmw_parts_search_view AS
WITH engine_compatibility AS (
    SELECT 
        comp.part_id,
        ARRAY_AGG(DISTINCT engine_code) as compatible_engines
    FROM bmw_part_compatibility comp
    CROSS JOIN LATERAL unnest(comp.engine_codes) AS engine_code
    GROUP BY comp.part_id
),
chassis_compatibility AS (
    SELECT 
        comp.part_id,
        ARRAY_AGG(DISTINCT ch.chassis_code) as compatible_chassis
    FROM bmw_part_compatibility comp
    JOIN bmw_chassis ch ON comp.chassis_id = ch.id
    GROUP BY comp.part_id
)
SELECT 
    p.id,
    ...
    c.category_code,
    c.category_name,
    COALESCE(cc.compatible_chassis, ARRAY[]::text[]) as compatible_chassis,
    COALESCE(ec.compatible_engines, ARRAY[]::text[]) as compatible_engines,
...
LEFT JOIN chassis_compatibility cc ON p.id = cc.part_id
LEFT JOIN engine_compatibility ec ON p.id = ec.part_id
GROUP BY p.id, p.part_number, p.part_name, p.description, p.price_msrp, p.is_discontinued, p.keywords, c.category_code, c.category_name, cc.compatible_chassis, ec.compatible_engines;
...
CREATE INDEX idx_bmw_parts_search_category ON bmw_parts_search_view(category_code);
CREATE INDEX idx_bmw_parts_search_discontinued ON bmw_parts_search_view(is_discontinued);
...
-- Create policies for public read access (optional - adjust as needed)
...
UNION ALL
SELECT 
    'BMW Transmissions' as table_name, 
    COUNT(*) as record_count 
...
UNION ALL
SELECT 
    'BMW Parts Search View' as table_name, 
    COUNT(*) as record_count 
FROM bmw_parts_search_view
ORDER BY table_name;

-- Test queries to verify everything works
SELECT 'Testing search view...' as status;
SELECT part_name, part_number, compatible_chassis, compatible_engines 
FROM bmw_parts_search_view 
LIMIT 5;
